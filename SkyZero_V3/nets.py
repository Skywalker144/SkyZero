...
# ---------------------------------------------------------------------------
# Config presets — (num_blocks, num_channels) → V4 ModelConfig.
# ---------------------------------------------------------------------------
...
def _build_config(num_blocks: int, num_channels: int) -> Dict:
    cfg = dict(_BASE_CONFIG)
    preset = _PRESET_CONFIGS.get((num_blocks, num_channels))
    if preset is not None:
        cfg.update(preset)
        return cfg

    cfg["trunk_num_channels"] = num_channels
    cfg["mid_num_channels"] = num_channels
    cfg["gpool_num_channels"] = max(16, num_channels // 4)
    cfg["p1_num_channels"] = max(12, num_channels // 3)
    cfg["g1_num_channels"] = max(12, num_channels // 3)
    cfg["v1_num_channels"] = max(12, num_channels // 3)
    cfg["v2_size"] = max(24, num_channels // 2)
    block_kind = []
    for i in range(num_blocks):
        name = f"rconv{i + 1}"
        block_type = "regulargpool" if (i + 2) % 3 == 0 else "regular"
        block_kind.append([name, block_type])
    cfg["block_kind"] = block_kind
    return cfg


# ---------------------------------------------------------------------------
# Activation and init helpers (KataGomo conventions).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# BiasMask — bias + mask, optional pre-scale.
# ---------------------------------------------------------------------------

class BiasMask(nn.Module):
    def __init__(self, c_in, config: Dict, is_after_batchnorm: bool = False):
        super().__init__()
        self.c_in = c_in
...
# ---------------------------------------------------------------------------
# NormMask — supports bnorm / brenorm / fixbrenorm / fixup / fixscale.
# ---------------------------------------------------------------------------
...
class NormMask(nn.Module):
    def __init__(
        ...
        # Defaults make brenorm behave as plain batch norm until ramp.
        self.renorm_avg_momentum = self.running_avg_momentum
        self.rmax = 1.0
        self.dmax = 0.0
...
class ResBlock(nn.Module):
    def __init__(self, name: str, c_main: int, c_mid: int, c_gpool: Optional[int], config: Dict, activation: str):
        super().__init__()
        self.norm_kind = config["norm_kind"]
...
class ValueHead(nn.Module):
    ...
    def forward(self, x, mask, mask_sum_hw, mask_sum: float):
        ...
        value_logits = self.linear_valuehead(outv2)
        return value_logits


# ---------------------------------------------------------------------------
# ResNet — top-level module, keeps V3 signature.
# ---------------------------------------------------------------------------

class ResNet(nn.Module):
    def __init__(self, game, num_blocks=6, num_channels=128):
        super().__init__()
        config = _build_config(num_blocks, num_channels)
        self.config = config
        self.norm_kind = config["norm_kind"]
        ...
        self.num_input_planes = game.num_planes
        self.activation = config.get("activation", "relu")

        if config.get("initial_conv_1x1", False):
            self.conv_spatial = nn.Conv2d(self.num_input_planes, self.c_trunk, kernel_size=1, bias=False)
        else:
            self.conv_spatial = nn.Conv2d(self.num_input_planes, self.c_trunk, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        policy_out = self.policy_head(out, mask=mask, mask_sum_hw=mask_sum_hw, mask_sum=mask_sum)
        value_logits = self.value_head(out, mask=mask, mask_sum_hw=mask_sum_hw, mask_sum=mask_sum)
