"""Train one epoch over data/shuffled/current/ for a single network.

One "epoch" here = TRAIN_STEPS_PER_EPOCH mini-batch SGD steps. After training,
saves model state_dict to data/nets/<network>/model_latest.pt and appends an
entry to data/nets/<network>/train.tsv. run.sh invokes this per network each
iter (all networks consume the same shared shuffle); export_model.py is then
invoked per network to produce a TorchScript artifact.
"""
...
def _load_or_init_model(args: TrainArgs) -> tuple[torch.nn.Module, int, dict | None, dict | None, dict | None, int]:
    ...
    ckpt_dir = args.data_dir / "nets" / args.network
    ...
    if latest.exists():
        ...
        print(f"{TAG} loaded checkpoint {latest} (global_step={global_step}"
        ...
        model.initialize()
        print(f"{TAG} starting from fresh model (no checkpoint found); model.initialize() called")
...
def main() -> int:
    ...
    print(f"{TAG} moving model to {args.device}...", flush=True)
    t = time.time()
    model = model.to(args.device)
    model.train()
    print(f"{TAG} model on {args.device} in {time.time() - t:.1f}s", flush=True)
    ...
    if optim_state is not None:
        try:
            ...
            print(f"{TAG} warning: failed to restore optimizer state ({e}); starting fresh")
    ...
    if lookahead_enabled:
        ...
        print(f"{TAG} Lookahead enabled: k={args.lookahead_k} alpha={args.lookahead_alpha} "
    ...
    if warmup_active:
        print(f"{TAG} LR warmup enabled: window={args.lr_warmup_samples} samples "
    ...
    if scaler_state is not None and use_amp:
        try:
            ...
            print(f"{TAG} warning: failed to restore GradScaler state ({e})")
    ...
    if args.enable_swa:
        ...
        if swa_state is not None:
            try:
                ...
                print(f"{TAG} warning: failed to restore SWA state ({e}); starting fresh")
                swa_accum_steps = 0
        print(f"{TAG} SWA enabled: scale={args.swa_scale} period_steps={args.swa_period_steps}")
    else:
        swa_accum_steps = 0
    ...
    # Progress print cadence: ~TRAIN_REPORT_COUNT (default 5) lines across the
    # epoch. Just a liveness/NaN check — real loss analysis is in view_loss.py.
    report_count = int(os.environ.get("TRAIN_REPORT_COUNT", "5"))
    report_every = max(1, args.train_steps // max(1, report_count))

    print(f"{TAG} warming up first batch (loading first shard)...", flush=True)
    t_warmup = time.time()
    first_batch = next(it)
    print(f"{TAG} first batch ready in {time.time() - t_warmup:.1f}s; "
    ...
    while step < args.train_steps:
        ...
        if step % report_every == 0 or step == args.train_steps:
            ...
            pct = 100 * step // args.train_steps
            print(f"{TAG} {args.network} step={step}/{args.train_steps} ({pct}%) "
                  f"loss={losses['total']:.2f} sps={sps:.0f}", flush=True)

    dt = time.time() - start
    avg = {k: v / max(1, step) for k, v in accum.items()}

    print(f"{TAG} {args.network} iter={args.iter} done: steps={step} "
          f"samples_seen={step * args.batch_size} loss={avg['total']:.3f} t={dt:.1f}s")
