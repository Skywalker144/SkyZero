...
class MCTS:
    ...
    def select(self, node, is_full_search=False):
        ...
        c_puct = c_puct_init + c_puct_log * math.log((node.n + 1 + c_puct_base) / c_puct_base)

        explore_scaling = c_puct * math.sqrt(node.n)
...
class AlphaZero:
    ...
    def _train_batch(self, batch):
        ...
        soft_policy_targets /= (torch.sum(soft_policy_targets, dim=-1, keepdim=True) + 1e-8)
