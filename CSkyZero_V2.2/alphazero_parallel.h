...
public:
    AlphaZeroParallel(
        ...
        : game_(game),
          ...
          cfg_(cfg),
          pcfg_(pcfg),
          replay_buffer_(game_.board_size, cfg_.min_buffer_size, cfg_.linear_threshold, cfg_.replay_alpha, cfg_.max_buffer_size, 50000, cfg_.data_dir),
    ...
    bool save_checkpoint(const std::string& filepath = "") {
        ...
        // Save replay buffer manifest (chunk files already on disk)
        replay_buffer_.flush();
        const fs::path rbm_path = fs::path(checkpoint_path).replace_extension(".rbm");
        replay_buffer_.save_manifest(rbm_path.string());
        checkpoint_archive.write("replay_buffer_manifest", torch::tensor({1}));  // marker
        ...
        const auto rbm_bytes = fs::file_size(rbm_path);
        ...
        std::cout << "Replay buffer manifest saved to " << rbm_path.string() << " (" << human_size(rbm_bytes) << ")\n";
        return true;
    }
    ...
    bool load_checkpoint(const std::string& filepath = "") {
        ...
        try {
            ...
            // Load replay buffer: manifest (.rbm), legacy .rb file, or legacy archive
            torch::Tensor rbm_marker;
            const bool has_manifest = checkpoint_archive.try_read("replay_buffer_manifest", rbm_marker);
            if (has_manifest) {
                // New format: load chunk manifest
                const fs::path rbm_path = fs::path(checkpoint_path).replace_extension(".rbm");
                replay_buffer_.load_manifest(rbm_path.string());
                std::cout << "Replay buffer manifest loaded from " << rbm_path.string() << "\n";
            } else {
                // Try legacy .rb file format
                ...
                if (has_rb_file) {
                    ...
                    replay_buffer_.load_from_legacy_rb(rb_path.string());
                    std::cout << "Replay buffer migrated from legacy .rb file\n";
                } else {
                    // Oldest legacy format: embedded archive
                    torch::serialize::InputArchive rb_archive;
                    checkpoint_archive.read("replay_buffer", rb_archive);

                    ReplayBuffer::LegacyState rb;
                    ...
                    replay_buffer_.load_legacy_state(rb);
                    std::cout << "Replay buffer migrated from legacy archive format\n";
                }
            }
    ...
    void learn() {
        ...
        if (session_start_time_.time_since_epoch().count() == 0) {
            ...
            initial_total_samples_ = total_samples_;
        }
...
private:
    ...
    void print_selfplay_stats() {
        ...
        const double sps = (elapsed > 0.0) ? (static_cast<double>(total_samples_ - initial_total_samples_) / elapsed) : 0.0;
    ...
    int64_t initial_total_samples_ = 0;
