    ...
    SelfplayResult selfplay_once(uint64_t seed) {
        ...
        while (!game_.is_terminal_canvas(state, last_action, last_player)) {
            ...
            if (cheap_this_move) {
                ...
                // recording the cheap row: skip root Dirichlet/Gumbel noise +
                // collapse root_fpu_* to non-root values (avoids the cheap
                // search wasting its small budget on wide-root exploration).
            ...
            // mcts.search dispatches internally on cfg_.search_algo, so the
            // same call site works for both Gumbel and PUCT modes.
            const auto sr = mcts.search(state, to_play, num_simulations, root, disable_root_noise_this_move);
