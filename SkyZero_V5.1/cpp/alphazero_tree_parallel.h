...
public:
    ...
    // Gumbel MCTS path. Same signature as ParallelMCTS<Game>::gumbel_search.
    ...
    // Public entry. Dispatches on cfg_.search_algo so existing callers keep
    // working unchanged. PUCT path: KataGomo-aligned vanilla PUCT MCTS.
    // num_threads_ workers each run one full PUCT descent → leaf NN eval →
    // backprop, repeated in chunks until num_simulations is consumed. Root
    // Dirichlet noise applied once per call; mcts_policy returned as raw
...
private:
    ...
    // KataGomo Search::addDirichletNoise. Called once per search() entry BEFORE
    // worker dispatch — no other thread is touching the tree at that point, so
    // prior writes are race-free without taking stripe mutexes.
    ...
    // KataGomo "removeRootNoise" bundle — set by search() on entry from
    // disable_root_noise, read by select_child when descending root.
