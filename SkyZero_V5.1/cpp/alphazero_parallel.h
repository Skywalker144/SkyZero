...
public:
    ...
    // Public entry. Dispatches on cfg_.search_algo so existing callers (which
    // all use `mcts.search(...)`) keep working unchanged: GUMBEL is V5's
    // legacy behavior; PUCT is the KataGomo-aligned path below.
    // Selfplay mode (PUCT): root Dirichlet noise + leaf-parallel batched
    // playouts (vloss) + PUCT descent + tree reuse. Output `mcts_policy` is
    // the raw root visit-count distribution; chosenMove temperature schedule
    // is applied by the caller. `gumbel_action` is the argmax-visit action
    // (field name retained for output compatibility).
    MCTSSearchOutput search(
        ...
        if (cfg_.search_algo == SearchAlgo::GUMBEL) {
            return gumbel_search(state, to_play, num_simulations, root, disable_root_noise);
        }
...
private:
    ...
    // infer + legal-mask + nn_policy_temperature + expand + backprop_path_with_vloss.
    // Caller built each PendingLeaf with vloss applied to the descent path.
    // sims_budget decremented once per successfully-expanded leaf.
    ...
    // Gumbel path: descend forced first action from `actions`, then PUCT to leaf.
    ...
    // KataGomo "removeRootNoise" bundle — set by search()/gumbel_search() on
