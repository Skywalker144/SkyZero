...
struct AlphaZeroConfig {
    ...
    // Dirichlet noise AND collapse root_fpu_{reduction_max,loss_prop} to the
    // non-root values, so 100-visit cheap searches don't waste budget on the
    // wide-root exploration meant for full searches.
    // KataGomo selfplay8b defaults: 0.75 / 200 / 0.0; selfplay1: 0.75 / 100 / 0.0.
    // V5.1 default 0.0 disables PCR (back-compat).
