...
// AB engine selector. PUCT path bypasses Gumbel sequential halving and runs
// vanilla AlphaZero PUCT; both paths consume the same num_simulations so AB
// matches at equal NN forward count.
enum class SearchAlgo { GUMBEL, PUCT };

static SearchAlgo parse_search_algo(
        const std::unordered_map<std::string, std::string>& m,
        SearchAlgo fallback = SearchAlgo::GUMBEL) {
    auto it = m.find("SEARCH_ALGO");
    if (it == m.end() || it->second.empty()) return fallback;
    const auto& v = it->second;
    if (v == "puct" || v == "PUCT" || v == "Puct") return SearchAlgo::PUCT;
    if (v == "gumbel" || v == "GUMBEL" || v == "Gumbel") return SearchAlgo::GUMBEL;
    return fallback;
}

static const char* search_algo_str(SearchAlgo a) {
    return a == SearchAlgo::PUCT ? "puct" : "gumbel";
}
...
int main(int argc, char** argv) {
    try {
        ...
        auto infer_batched = [&](const std::vector<std::vector<int8_t>>& batch) {
            return server.infer_batch(batch);
        };

        const SearchAlgo algo_a = parse_search_algo(cfg_a_map, SearchAlgo::GUMBEL);
        const SearchAlgo algo_b = parse_search_algo(cfg_b_map, SearchAlgo::GUMBEL);
        ...
        std::cerr << "[gomoku_ab] model=" << cli.model << "\n"
                  ...
                  << " threads_a=" << threads_a
                  << " threads_b=" << threads_b
                  << " algo_a=" << search_algo_str(algo_a)
                  << " algo_b=" << search_algo_str(algo_b)
        ...
        auto worker = [&](int tid) {
            try {
                ...
                // ParallelMCTS instead of TreeParallelMCTS in this AB-PUCT
                // build: PUCT path is single-thread leaf-batched (same backend
                // as the gumbel path it shares), so both engines have identical
                // threading semantics when comparing strength.
                // 3rd arg is leaf_batch_size (== chunk for both gumbel SH
                // phases and run_puct_rollouts), reusing the cfg key
                // SEARCH_THREADS_PER_TREE for source-compat with V5's a/b.cfg.
                ParallelMCTS<Gomoku> mcts_a(game, cfg_a, threads_a, infer,
                                            infer_batched, rng());
                ParallelMCTS<Gomoku> mcts_b(game, cfg_b, threads_b, infer,
                                            infer_batched, rng());
                ...
                while (!abort_flag.load()) {
                    ...
                    while (!game.is_terminal(state, last_action, last_player)) {
                        ...
                        const SearchAlgo algo = a_to_move ? algo_a : algo_b;
                        const auto res = (algo == SearchAlgo::PUCT)
                            ? mcts.puct_search(state, to_play, sims, root)
