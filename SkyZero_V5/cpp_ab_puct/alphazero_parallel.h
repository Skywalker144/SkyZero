...
public:
    ...
    // Vanilla AlphaZero PUCT search (no Gumbel sequential halving). Used by
    // gomoku_ab_puct_main for AB tests against gumbel_search at the same
    // simulation budget. Root noise is intentionally omitted (caller-side
    // policy: AB evaluation runs noise-off on both engines for fairness).
    // `gumbel_action` in the output is repurposed as the argmax-visit action.
    MCTSSearchOutput puct_search(
        const std::vector<int8_t>& state,
        int to_play,
        int num_simulations,
        std::unique_ptr<MCTSNode>& root
        ...
        if (!root->is_expanded()) {
            ...
            // forbidden). Caller treats empty policy as a draw.
            out.nn_policy = std::move(nn_policy);
            out.nn_value_probs = nn_value_probs;
        ...
        // Reuse the same leaf_batch_size_ that gumbel_sequential_halving uses
        // for its per-phase chunking so both engines submit equally-sized
        // leaf batches to the shared inference server.
        ...
        for (const auto& c : root->children) {
            ...
            out.visit_counts[static_cast<size_t>(a)] = static_cast<float>(c->n);
            total_visits += c->n;
            if (c->n > best_n) { best_n = c->n; best_action = a; }
        }
        if (total_visits > 0) {
...
private:
    ...
    void run_rollouts(MCTSNode& root, const std::vector<int>& actions, int& sims_budget) {
        ...
        process_pending_leaves(pending, sims_budget);
    }

    // PUCT-from-root rollout collector. Mirrors run_rollouts but with no
    // forced first action: every step (including from root) uses select().
    // Identical post-collection processing via process_pending_leaves.
    void run_puct_rollouts(MCTSNode& root, int count, int& sims_budget) {
        ...
        for (int k = 0; k < count; ++k) {
            ...
            if (node == &root) {
                // Root unexpanded — caller did root_expand already, so this
                // means is_expanded() flipped under us. Defensive cleanup.
                remove_vloss_on_path(path);
                continue;
            }
            ...
            pl.leaf = node;
            pl.path = std::move(path);
            pl.encoded = game_.encode_state_v5(node->state, node->to_play);
    ...
    // Shared leaf-batch inference + expand + backprop. Extracted from
    // run_rollouts so run_puct_rollouts can reuse it bit-for-bit. Behavior
    // is unchanged from the original inline version in run_rollouts.
    void process_pending_leaves(std::vector<PendingLeaf>& pending, int& sims_budget) {
