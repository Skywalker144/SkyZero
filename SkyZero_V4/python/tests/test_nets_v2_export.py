import pytest
import torch
from nets_v2 import build_b8c96, KataGoNetForExport, _unflatten_export_output
...
def test_torchscript_trace_b8c96():
    ...
    # Wrap model for export (flattens tuple outputs to all-tensor dict)
    export_model = KataGoNetForExport(model)
    export_model.eval()
    ...
    with torch.no_grad():
        scripted = torch.jit.trace(export_model, (example_state, example_global), strict=False)

    # Trace 后再调一次, 输出形状应一致
    out_eager = export_model(example_state, example_global)
