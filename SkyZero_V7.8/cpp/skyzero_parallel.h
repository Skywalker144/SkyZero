...
public:
    ...
    MCTSSearchOutput search(
        ...
        // V7.8: a fastSearch dispatches on the fast algos (default = full's).
        const auto eff_root_algo =
            fast_search_ ? cfg_.fast_root_search_algo : cfg_.root_search_algo;
        if (eff_root_algo == SkyZeroConfig::RootSearchAlgo::kPuct) {
...
private:
    ...
    MCTSNode* select(MCTSNode& node) {
        // V7.8: a fastSearch descends with the fast algo (default = full's).
        const auto eff_non_root_algo =
            fast_search_ ? cfg_.fast_non_root_search_algo : cfg_.non_root_search_algo;
        if (eff_non_root_algo == SkyZeroConfig::NonRootSearchAlgo::kGumbel) {
