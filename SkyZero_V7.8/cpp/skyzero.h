...
struct SkyZeroConfig {
    ...
    // Fast (PCR cheap) search algos — V7.8 hybrid PCR. A fastSearch builds its
    // MCTS dispatch from these instead of root/non_root_search_algo, so the
    // cheap search can run a different algorithm than the full search (e.g.
    // full=puct, fast=gumbel). selfplay_main defaults each to the resolved
    // full-search algo, so a config that doesn't set them behaves exactly as
    // before. When they differ from the full algos, selfplay decouples the
    // tree (only full→full reuses; see selfplay_manager.h). Selfplay-only —
    // fastSearch is selfplay-only, so the other backends never read these.
    RootSearchAlgo fast_root_search_algo = RootSearchAlgo::kGumbel;
    NonRootSearchAlgo fast_non_root_search_algo = NonRootSearchAlgo::kPuct;

    // KataGo clearBotBeforeSearch (play.cpp:1875-1879, forced true in KataGo
    // selfplay; play.cpp:1071 clearSearch()). When true, a FULL (recorded)
    // search starts from a CLEARED tree so its policy target is a clean
    // N-visit search with effective root Dirichlet noise; only a cheap
    // (non-recorded, fastSearch) search reuses the carried tree — and only when
    // its algo matches the tree's (see fast_*_search_algo). SkyZero's historical
    // default is FALSE (reuse before full searches too); enable for KataGo
    // fidelity. Selfplay-only. Read by selfplay_manager.
    bool clear_tree_before_full_search = false;
    ...
    void validate() const {
        ...
        if (fast_root_search_algo == RootSearchAlgo::kPuct &&
            fast_non_root_search_algo == NonRootSearchAlgo::kGumbel) {
            throw std::runtime_error(
                "FAST_ROOT_SEARCH_ALGO=puct requires FAST_NON_ROOT_SEARCH_ALGO=puct");
        }
    }
