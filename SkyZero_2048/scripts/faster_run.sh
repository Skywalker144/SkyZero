#!/usr/bin/env bash
# Async orchestration loop for 2048 Stochastic Gumbel AlphaZero.
#
# This is the throughput-oriented alternative to scripts/run.sh. run.sh is a
# strict per-iteration BARRIER: it runs a bounded self-play batch and waits for
# the LAST (longest) game before training, so the GPU/CPU idle through the long-
# game tail (batches shrink, sps collapses). faster_run.sh removes that barrier
# with the canonical async-AlphaZero producer/consumer split:
#
#   PRODUCER : one selfplay2048_par --daemon per GPU (incl. MAIN_GPU). It never
#              hits a games barrier — finished game slots refill from an
...
#              export bumps models/latest.pt mtime -> daemons hot-reload -> new
#              version. The GPU is never idle: the producer keeps generating
#              while the (tiny 4x4 net) trainer shares the card.
#
...
# cores for the trainer (lower THREADS a touch) so MCTS workers and the
# DataLoader don't starve each other.
#
# Usage: CONFIG_DIR=configs/vtransform bash scripts/faster_run.sh [max_iters]
#   max_iters (CLI arg) or ITERS (run.cfg) caps trained iters; MAX_TIME_SECONDS caps time.
#
# Reads the same run.cfg as run.sh, plus these async-only knobs (all optional):
#   ASYNC_ROWS_PER_CYCLE   new rows that must accumulate before a train cycle
#                          (default ~ TRAIN_SAMPLES_PER_EPOCH/MAX_TRAIN_PER_DATA = one epoch)
...
# Keep C++ binaries in sync (configure-if-needed + incremental build).
...
if [[ "$max_iter" -ge 0 ]]; then
    ...
    if [[ "${#LAGGING[@]}" -gt 0 ]] && compgen -G "$DATA_DIR/shuffled/current/*" > /dev/null; then
        ...
        [[ -z "$CATCHUP_STEPS" || "$CATCHUP_STEPS" -le 0 ]] && CATCHUP_STEPS=$(( ${TRAIN_SAMPLES_PER_EPOCH:-3840000} / ${BATCH_SIZE:-128} ))
...
echo "[faster_run] starting self-play daemon(s) on GPUs: $ASYNC_SELFPLAY_GPUS"
...
# --- async knobs ---
SPE="${TRAIN_SAMPLES_PER_EPOCH:-3840000}"
MPD="${MAX_TRAIN_PER_DATA:-4.0}"
ASYNC_ROWS_PER_CYCLE="${ASYNC_ROWS_PER_CYCLE:-$(awk -v s="$SPE" -v m="$MPD" 'BEGIN{printf "%d", s/m}')}"
...
echo "[faster_run] consumer: rows_per_cycle=$ASYNC_ROWS_PER_CYCLE poll=${ASYNC_POLL_SECONDS}s resume_iter=$iter"

# Token bucket (faithful port of bucket.py math; state in BUCKET_STATE).
# Args: new_rows. Prints train_steps. Updates BUCKET_STATE (cum_rows bucket_level).
compute_train_steps() {
    local new_rows="$1"
    local maxb="${MAX_TRAIN_BUCKET_SIZE:-$((2 * SPE))}"
    awk -v nr="$new_rows" -v spe="$SPE" -v bs="${BATCH_SIZE:-128}" -v mpd="$MPD" \
        -v maxb="$maxb" -v sf="$BUCKET_STATE" '
    BEGIN{
        cum=0; bucket=spe;                       # bucket.py init: level = one epoch
...
while true; do
    # Time cap (checked on every pass so a slow data rate still honors it).
