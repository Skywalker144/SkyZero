...
class KataGoNet(nn.Module):
    ...
    def set_gamma_center_one(self, center_one: bool) -> None:
        """设置 fixscaleonenorm 的 gamma 约定 (见 full_nets.KataGoNet.set_gamma_center_one).

        True : (gamma+1)*scale — 当代 KataGo / b40 / 自家从零训练默认.
        False: gamma*scale — 旧约定 (如官方 b28c512nbt), 仅加载该约定权重时调.
        """
...
def build_b4c64(activation: str = "mish") -> KataGoNet:
    """4 块 × 64 trunk (显式 num_channels); head 宽度由 NetConfig 按 KataGo 风格规则推导."""
...
def build_b8c96(activation: str = "mish") -> KataGoNet:
    """8 块 × 96 trunk (显式 num_channels); head 宽度由 NetConfig 按 KataGo 风格规则推导."""
    from model_config import NetConfig
...
def build_b12c128(activation: str = "mish") -> KataGoNet:
    """12 块 × 128 trunk (显式 num_channels); head 宽度由 NetConfig 按 KataGo 风格规则推导."""
    from model_config import NetConfig
