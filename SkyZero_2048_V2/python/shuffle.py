...
def _slice_batch(b: NpzBatch, s: int, e: int) -> NpzBatch:
    # 2048 NpzBatch schema (data_processing.py): state / policy_target /
    # value_target / sample_weight. (Mainline also slices global_features /
    # opponent_policy / td_value / futurepos here — none exist in 2048.)
    return NpzBatch(
        state=b.state[s:e],
