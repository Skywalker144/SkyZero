...
class Config:
    ...
    value_hidden: int = 64
    # Opt-in: use the faithful KataGo nested-bottleneck net (full_nets.Net2048NBT,
    # fixscale norm + RepVGG init) instead of the compact BatchNorm Net2048.
    full_net: bool = False
    ...
    # stays in one unit.
    value_scale: float = 30.0   # h-SPACE scale: head regresses h(raw)/VALUE_SCALE
    # The value head ALWAYS uses the invertible MuZero scaling h() (target =
    # h(raw)/SCALE, decode = h_inv(out*SCALE); see value_transform.py). h() is
    # hardcoded on (no toggle), so VALUE_SCALE lives in h-space (~30).
...
def config_from_env(**overrides) -> Config:
    ...
    kw = dict(
        ...
        full_net=_env_int("FULL_NET", 0) != 0,
