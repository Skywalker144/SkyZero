...
_PANELS = {
    "loss": _plot_loss,
    "selfplay": _plot_selfplay,
    "eval": _plot_eval,
    "probe": _plot_probe,
}


def make_plots(data_dir, only=None) -> None:
    """Render PNGs. Best-effort: import/plot errors are swallowed so a plotting
    failure never aborts the training loop. `only` (iterable of panel names)
    restricts which panels render; unknown names (e.g. mainline's gomoku
    'ratio') are silently skipped — 2048 has no ratio panel."""
    ...
    try:
        ...
        return
    names = list(_PANELS) if not only else [n for n in only if n in _PANELS]
    for name in names:
        try:
            _PANELS[name](data_dir, plt)
        except Exception as e:  # noqa: BLE001
            print(f"[plot] {name} failed: {e}", file=sys.stderr)


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--data-dir", default="data2048_nbt")
    # --plot accepted for run.sh / mainline CLI compatibility (always plots).
    ap.add_argument("--plot", action="store_true")
    ap.add_argument("--tail", type=int, default=20)  # accepted, unused (no console summary)
    ap.add_argument("--only", default="",
                    help="comma list of panels: loss,selfplay,eval,probe (others skipped)")
    args = ap.parse_args()
    only = [s.strip() for s in args.only.split(",") if s.strip()] or None
    make_plots(args.data_dir, only=only)
    return 0
