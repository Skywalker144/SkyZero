...
append a logs/selfplay.tsv row at model-reload boundaries, so tsv-based row
accounting can't tell the consumer "how much fresh data exists yet" (cold-start
deadlock: no reload until we train, no train until we see rows). Instead the
consumer polls the disk directly: this script sums the rows of every
selfplay/*.npz whose st_mtime is strictly greater than --since.
