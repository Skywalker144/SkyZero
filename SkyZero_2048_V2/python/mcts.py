...
def net_evaluator(net, cfg: Config, device: str | None = None) -> EvalFn:
    ...
    points). value head outputs scaled units => multiply by value_scale, then
    invert the MuZero h() (always on) to get back to raw points."""
    import torch
    ...
    def _eval(states: list[np.ndarray]):
        ...
        v = value_transform.from_h_np(v).astype(np.float32)   # decode MuZero h^-1 -> raw points
