...
class Net2048(nn.Module):
    ...
    # No-ops so callers (train/export/init/evaluate) can invoke initialize() /
    # set_norm_scales() unconditionally — BatchNorm self-normalizes, so unlike
    # the fixscale nbt net (full_nets.Net2048NBT) there are no scales to set.
...
def build_net(cfg: Config) -> nn.Module:
    """Compact BatchNorm net by default; the faithful KataGo nested-bottleneck
    (fixscale + RepVGG init) when cfg.full_net (FULL_NET=1). Both expose the same
    forward(x) -> (policy[B,4], value[B]) contract + initialize()/set_norm_scales()."""
    if getattr(cfg, "full_net", False):
        from full_nets import build_full_net
        return build_full_net(cfg)
    return Net2048(cfg)
