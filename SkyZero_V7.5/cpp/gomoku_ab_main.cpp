...
static SkyZeroConfig build_mcts_cfg(
    ...
    c.non_root_search_algo = SkyZeroConfig::parse_non_root_search_algo(
    ...
    c.root_desired_per_child_visits_coeff =
        cfg_get<float>(m, "ROOT_DESIRED_PER_CHILD_VISITS_COEFF", 0.0f);
    c.chosen_move_temperature = cfg_get<float>(m, "CHOSEN_MOVE_TEMPERATURE", 0.0f);
    c.chosen_move_temperature_early =
        cfg_get<float>(m, "CHOSEN_MOVE_TEMPERATURE_EARLY", 0.0f);
    c.chosen_move_temperature_halflife =
        cfg_get<float>(m, "CHOSEN_MOVE_TEMPERATURE_HALFLIFE", 19.0f);
    c.validate();
