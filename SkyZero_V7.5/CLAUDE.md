...
3. `compute_selfplay_target.py --order` → 按累计目标账本算本 iter 的 selfplay 产量(gate 化自适应 selfplay,`TARGET_REPLAY_RATIO` 是软上限非稳态)
4. `selfplay.sh`(主 GPU)+ 多 GPU 时 `selfplay_daemon.sh`(副卡并行)→ npz 落到 `data/selfplay/`
5. `shuffle.sh` → KataGo 三参数 power-law 窗口采样 → `data/shuffled/current/`;磁盘行数低于 `MIN_ROWS` 时退出码 2 = 本 iter **跳过训练**,selfplay 继续累积
...
- **固定 train + gate 化自适应 selfplay**(`python/compute_selfplay_target.py`,V7.3 起替代旧 token bucket):
  train 每 iter 固定步数;selfplay 产量由累计目标账本 gate,状态在 `data/selfplay_target.json`(消耗一次语义)。
- **Selfplay 窗口**(`python/shuffle.py`):`W(N) = (N^E - M^E) / (E * M^(E-1)) * IWPR + M`,
  即 `MIN_ROWS(M) / TAPER_WINDOW_EXPONENT(E) / EXPAND_WINDOW_PER_ROW(IWPR)`。
  `N` 是**累计**产量(`logs/selfplay.tsv` 台账,V7.5 修复:OOW 清理删窗口外文件但窗口本身不缩)。磁盘行数 `< M` 时 shuffle 跳过。
...
- **续跑(resume)**:run.sh 读每个 net 的 `state.json`,取 `max(iter)` 作为下一 iter;
  落后的 net 在已有 `shuffled/current/` 上做一次 **catch-up 训练**后才进主循环(order/selfplay/shuffle 不能重算)。
...
同名变量优先级:`.local` > cfg 默认值。注意 `run.cfg` 是平赋值(C++ `parse_cfg()` 也要读它,
不能写成 `${VAR:-}`),source 时会**覆盖**调用前 export 的 env —— env 注入只对 `${VAR:-}`
形式的键(如 `paths.cfg` 的 `DATA_DIR`)有效,其余覆盖一律走 `.local`。
...
**Python**(`python/`):
- 训练流水线:`train.py` / `compute_selfplay_target.py` / `shuffle.py` / `schedule.py` / `warmup.py`
...
- **训练状态文件有"消耗一次"语义**:`selfplay_target.json` / `state.json` / `schedule.tsv` /
...
3. **跑 `run.sh` 前**,如果改了任何 `python/{full_nets,nets,model_config,compute_selfplay_target,shuffle,schedule,train}.py`,
