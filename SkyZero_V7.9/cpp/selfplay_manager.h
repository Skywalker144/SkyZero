...
private:
    ...
    SelfplayResult selfplay_once(uint64_t seed) {
        ...
        // V7.8 tree-reuse policy (computed once; applied before each search).
        //  - decoupled_fast_tree: the fast (cheap) search uses a different algo
        //    than the full search, so neither can coherently reuse a tree the
        //    other built (gumbel SH vs puct) → that arm never reuses. False for
        //    any config that doesn't override FAST_*_SEARCH_ALGO.
        //  - clear_tree_before_full_search (KataGo clearBotBeforeSearch,
        //    play.cpp:1875-1879): a FULL/recorded search clears first (clean
        //    N-visit target + effective root Dirichlet noise); only a cheap
        //    search reuses. Default off = SkyZero legacy reuse-across-all-moves
        //    (what every non-HybridPCR experiment relies on).
        const bool decoupled_fast_tree =
            cfg_.fast_root_search_algo != cfg_.root_search_algo ||
            cfg_.fast_non_root_search_algo != cfg_.non_root_search_algo;
        ...
        while (!game_.is_terminal_canvas(state, last_action, last_player)) {
            ...
            if (cfg_.fast_search_prob > 0.0f) {
                std::uniform_real_distribution<float> u01f(0.0f, 1.0f);
                fast_search = u01f(worker_rng) < cfg_.fast_search_prob;
            }
            if (fast_search) {
                // Fixed 1/6 of the full budget (KataGo 600/100), so the ratio
                // holds across warmup NUM_SIMULATIONS stages; exploration is
            ...
            // Apply the tree-reuse policy (see above) before searching:
            //   decoupled              -> never reuse (mixed algos can't share a tree)
            //   else clear-before-full -> full clears, cheap reuses (KataGo)
            //   else (legacy)          -> reuse across all moves (the reroot below)
            bool reuse_tree;
            if (decoupled_fast_tree)                     reuse_tree = false;
            else if (cfg_.clear_tree_before_full_search) reuse_tree = fast_search;
            else                                         reuse_tree = true;
            if (!reuse_tree) root.reset(new MCTSNode(state, to_play));

            const auto sr = mcts.search(state, to_play, num_simulations, root, fast_search);
            ...
            // Defensive: Gumbel SH guarantees gumbel_action >= 0 for any legal
            // position (falls back to argmax(improved_policy)). This branch
            // exists only so a corrupted MCTS state can't crash selfplay.
            if (action < 0) {
