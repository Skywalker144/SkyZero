...
int main(int argc, char** argv) {
    try {
        ...
        // V7.8 hybrid PCR: per-search-type algos for the fast (cheap) search.
        // Default to the full-search algos, so configs that don't set them
        // behave exactly as before. When they differ (e.g. full=puct,
        // fast=gumbel) selfplay decouples the tree (selfplay_manager.h).
        cfg.fast_non_root_search_algo = SkyZeroConfig::parse_non_root_search_algo(
            cfg_get<std::string>(cfg_map, "FAST_NON_ROOT_SEARCH_ALGO",
                cfg_get<std::string>(cfg_map, "NON_ROOT_SEARCH_ALGO", "puct")));
        cfg.fast_root_search_algo = SkyZeroConfig::parse_root_search_algo(
            cfg_get<std::string>(cfg_map, "FAST_ROOT_SEARCH_ALGO",
                cfg_get<std::string>(cfg_map, "ROOT_SEARCH_ALGO", "gumbel")));
        // KataGo clearBotBeforeSearch — clear the tree before every full
        // (recorded) search. Default false = SkyZero legacy (reuse before full
        // too); HybridPCR sets it true for KataGo-faithful PCR.
        cfg.clear_tree_before_full_search =
            cfg_get_bool(cfg_map, "CLEAR_TREE_BEFORE_FULL_SEARCH", false);
        // PUCT-root knobs. Defaults mirror KataGo's selfplay configs
        // (KataGo/cpp/configs/training/selfplay*.cfg); all inert while
        // ROOT_SEARCH_ALGO=gumbel.
        {
            const int mbs = cfg_get<int>(cfg_map, "MAIN_BOARD_SIZE", 15);
        ...
        // Playout Cap Randomization (KataGo cheapSearch → fastSearch).
        // FAST_SEARCH_PROB is the switch: 0 (default) = off. fast budget is
        // fixed at 1/6 of the full budget; KataGo selfplay uses prob 0.75,
        // target weight 0.
